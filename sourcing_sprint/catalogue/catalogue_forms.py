...
def make_multiselect(key, label, options, format_func=lambda x:x, help="", default=None):
...
def form_general_info_add(entry_dict, options):
    with st.expander("General information", expanded=False):
        ...
        entry_dict["type"] = st.radio(
            ...
            index=list(options["entry_types"].keys()).index(st.session_state.save_state.get("add_description_type", "primary"))
        )
...
def select_entry_val(catalogue, options):
    with st.expander("Select catalogue entry to validate", expanded=False):
        ...
        st.markdown(f"##### Validating: {options['entry_types'].get(entry_dict['type'], '')} - {entry_dict['description']['name']}\n\n{entry_dict['description']['description']}")
    return entry_dict
...
def form_languages_add(entry_dict, options, countries, region_tree):
    with st.expander("Language names and represented regions", expanded=False):
        ...
        entry_dict["languages"]["language_names"] = make_multiselect(
            ...
            format_func=lambda x: options["language_lists"]["language_groups"].get(x, ""),
        ...
        if make_checkbox(
            ...
            entry_dict["languages"]["language_names"] += make_multiselect(
                ...
                options=[", ".join(x["description"]) for x in options["languages_bcp47"]],
...
def form_languages_val(entry_dict, options, countries, region_tree):
    with st.expander("Validate language names and represented regions", expanded=False):
        ...
        region_choices = sorted(set(
            list(region_tree.keys()) + countries + entry_dict["languages"]["language_locations"]
        )) + ["other"]
...
def form_custodian(entry_dict, options, countries, catalogue, mode):
    with st.expander(
        ...
        st.markdown((entry_organization_text if entry_dict["type"] == "organization" else entry_custodian_text))
        if entry_dict["type"] == "organization":
            entry_dict["custodian"]["in_catalogue"] = ""
        else:
            organization_catalogue = dict([(entry['uid'], entry) for entry in catalogue.values() if entry["type"] == "organization"])
            ...
            entry_dict["custodian"]["in_catalogue"] = make_selectbox(
                ...
                index=organization_keys.index(entry_dict["custodian"].get("in_catalogue", "")) if mode == "val" else None,
            )
        if entry_dict["custodian"]["in_catalogue"] == "":
            ...
            if mode == "add":
                ...
                custodian_type_options = [""] + sorted(set(
                    options["custodian_types"] + [entry_dict["custodian"]["type"]]
                )) + ["other"]
            entry_dict["custodian"]["type"] = make_selectbox(
                ...
                index=custodian_type_options.index(entry_dict["custodian"]["type"]) if mode == "val" else None,
            )
            ...
            entry_dict["custodian"]["location"] = make_selectbox(
                ...
                index=([""] + countries).index(entry_dict["custodian"]["location"]) if mode == "val" else None,
            )
            entry_dict["custodian"]["contact_name"] = make_text_input(
                ...
                value=entry_dict["custodian"]["contact_name"] if mode == "val" else entry_dict["description"]["name"],
            )
            entry_dict["custodian"]["contact_email"] = make_text_input(
                ...
                value=entry_dict["custodian"]["contact_email"] if mode == "val" else None,
            )
...
def form_availability(entry_dict, options, mode):
    ...
    with st.expander("Obtaining the data: online availability and data owner/custodian", expanded=False):
        ...
        entry_dict["availability"]["procurement"]["for_download"] = make_radio(
            ...
            index=download_options.index(entry_dict["availability"]["procurement"]["for_download"]) if mode == "val" else None,
        )
        if "Yes -" in entry_dict["availability"]["procurement"]["for_download"]:
            entry_dict["availability"]["procurement"]["download_url"] = make_text_input(
                ...
                value=entry_dict["availability"]["procurement"]["download_url"] if mode == "val" else None,
            )
        else:
            entry_dict["availability"]["procurement"]["download_email"] = make_text_input(
                ...
                value=entry_dict["availability"]["procurement"]["download_email"] if mode == "val" else entry_dict["custodian"]["contact_email"],
    ...
    with st.expander("Data licenses and Terms of Service", expanded=False):
        ...
        entry_dict["availability"]["licensing"]["has_licenses"] = make_radio(
            ...
            index=["Yes", "No", "Unclear"].index(entry_dict["availability"]["licensing"]["has_licenses"]) if mode == "val" else None,
        )
        if entry_dict["availability"]["licensing"]["has_licenses"] == "Yes":
            entry_dict["availability"]["licensing"]["license_properties"] = make_multiselect(
                ...
                default=entry_dict["availability"]["licensing"]["license_properties"] if mode == "val" else None,
            )
            st.markdown(
            ...
            entry_dict["availability"]["licensing"]["license_list"] = make_multiselect(
                ...
                default=entry_dict["availability"]["licensing"]["license_list"] if mode == "val" else None,
            )
            entry_dict["availability"]["licensing"]["license_text"] = make_text_area(
                ...
                value=entry_dict["availability"]["licensing"]["license_text"] if mode == "val" else None,
            )
        else:
            entry_dict["availability"]["licensing"]["license_text"] = make_text_area(
                key=f"{mode}_availability_licensing_license_text",
    ...
    with st.expander("Personal Identifying Information", expanded=False):
        ...
        if entry_dict["availability"]["pii"]["has_pii"] == "Yes":
            ...
            pii_likely_ranks = ["", "very likely", "somewhat likely", "unlikely", "none"]
            entry_dict["availability"]["pii"]["generic_pii_likely"] = make_selectbox(
                ...
                index=pii_likely_ranks.index(entry_dict["availability"]["pii"]["generic_pii_likely"]) if mode == "val" else None,
            ...
            if "likely" in entry_dict["availability"]["pii"]["generic_pii_likely"]:
                entry_dict["availability"]["pii"]["generic_pii_list"] = make_multiselect(
                    ...
                    default=entry_dict["availability"]["pii"]["generic_pii_list"] if mode == "val" else None,
            ...
            entry_dict["availability"]["pii"]["numeric_pii_likely"] = make_selectbox(
                ...
                options=pii_likely_ranks,
                index=pii_likely_ranks.index(entry_dict["availability"]["pii"]["numeric_pii_likely"]) if mode == "val" else None,
            ...
            if "likely" in entry_dict["availability"]["pii"]["numeric_pii_likely"]:
                entry_dict["availability"]["pii"]["numeric_pii_list"] = make_multiselect(
                    ...
                    default=entry_dict["availability"]["pii"]["numeric_pii_list"] if mode == "val" else None,
            ...
            entry_dict["availability"]["pii"]["sensitive_pii_likely"] = make_selectbox(
                ...
                options=pii_likely_ranks,
                index=pii_likely_ranks.index(entry_dict["availability"]["pii"]["sensitive_pii_likely"]) if mode == "val" else None,
            ...
            if "likely" in entry_dict["availability"]["pii"]["sensitive_pii_likely"]:
                entry_dict["availability"]["pii"]["sensitive_pii_list"] = make_multiselect(
                    ...
                    default=entry_dict["availability"]["pii"]["sensitive_pii_list"] if mode == "val" else None,
            ...
            entry_dict["availability"]["pii"]["no_pii_justification_class"] = make_radio(
                ...
                index=no_pii_options.index(entry_dict["availability"]["pii"]["no_pii_justification_class"]) if mode == "val" else None,
            )
            if entry_dict["availability"]["pii"]["no_pii_justification_class"] == "other":
                entry_dict["availability"]["pii"]["no_pii_justification_text"] = make_text_area(
                    ...
                    value=entry_dict["availability"]["pii"]["no_pii_justification_text"] if mode == "val" else None,
                )
    if mode == "val":
        st.markdown("If you are satisfied with the values for the fields above, press the button below to update and validate the **availability** section of the entry")
...
def form_source_category(entry_dict, options, mode):
    ...
    with st.expander("Source category", expanded=False):
        entry_dict["source_category"]["category_type"] = make_selectbox(
            ...
            index=["", "collection", "website", "other"].index(entry_dict["source_category"]["category_type"]) if mode == "val" else None,
        )
        if entry_dict["source_category"]["category_type"] == "website":
            entry_dict["source_category"]["category_web"] = make_selectbox(
                ...
                index=([""] + options["primary_taxonomy"]["website"]).index(entry_dict["source_category"]["category_web"]) if mode == "val" else None,
            )
        else:
            entry_dict["source_category"]["category_web"] = ""
        if entry_dict["source_category"]["category_type"] == "collection" or "collection" in entry_dict["source_category"]["category_web"]:
            entry_dict["source_category"]["category_media"] = make_selectbox(
                ...
                index=([""] + options["primary_taxonomy"]["collection"]).index(entry_dict["source_category"]["category_media"]) if mode == "val" else None,
            )
        else:
    ...
    if mode == "val":
        st.markdown("If you are satisfied with the values for the fields above, press the button below to update and validate the **source category** section of the entry")
        if make_checkbox(key="validated_source_category", label="Validate: source category"):
...
def form_processed_from_primary(entry_dict, options, catalogue, mode):
    ...
    with st.expander("List primary sources", expanded=False):
        ...
        entry_dict["processed_from_primary"]["from_primary"] = make_radio(
            ...
            index=["Taken from primary source", "Original data"].index(entry_dict["processed_from_primary"]["from_primary"]) if mode == "val" else None,
        )
        if entry_dict["processed_from_primary"]["from_primary"] == "Taken from primary source":
            ...
            entry_dict["processed_from_primary"]["primary_availability"] = make_radio(
                ...
                index=primary_available_options.index(entry_dict["processed_from_primary"]["primary_availability"]) if mode == "val" else None,
            )
            if entry_dict["processed_from_primary"]["primary_availability"] != "No - the dataset curators kept the source data secret":
                primary_catalogue = dict([(entry['uid'], entry) for entry in catalogue.values() if entry["type"] == "primary"])
                primary_keys = list(primary_catalogue.keys())
                entry_dict["processed_from_primary"]["from_primary_entries"] = make_multiselect(
                    ...
                    default=entry_dict["processed_from_primary"]["from_primary_entries"] if mode == "val" else None,
                )
            if "Yes" in entry_dict["processed_from_primary"]["primary_availability"]:
                entry_dict["processed_from_primary"]["primary_types"] = make_multiselect(
                    ...
                    options=[f"web | {w}" for w in options['primary_taxonomy']["website"]] + options['primary_taxonomy']["collection"],
                    default=entry_dict["processed_from_primary"]["primary_types"] if mode == "val" else None,
                )
                ...
                entry_dict["processed_from_primary"]["primary_license"] = make_radio(
                    ...
                    index=primary_license_options.index(entry_dict["processed_from_primary"]["primary_license"]) if mode == "val" else None,
                )
    if mode == "val":
        st.markdown("If you are satisfied with the values for the fields above, press the button below to update and validate the **processed primary source** section of the entry")
        if make_checkbox(key="validated_processed_from_primary", label="Validate: processed primary source"):
...
def form_media(entry_dict, options, mode):
    ...
    with st.expander("Media type", expanded=False):
        ...
        if "text" in entry_dict["media"]["category"]:
            text_format_dict = dict(
                [(fmt, desc) for fmt, desc in list(options['file_formats']["Text"].items()) + \
                    list(options['file_formats']["Web"].items()) + [("other", "other text file format")]]
            )
            if mode == "val":
            ...
            entry_dict["media"]["text_is_transcribed"] = make_radio(
                ...
                index=["No", "Yes - audiovisual", "Yes - image"].index(entry_dict["media"]["text_is_transcribed"]) if mode == "val" else None,
            )
        if "audiovisual" in entry_dict["media"]["category"] or "audiovisual" in entry_dict["media"]["text_is_transcribed"]:
            audiovisual_format_dict = dict(
                [(fmt, desc) for fmt, desc in list(options['file_formats']["Audio"].items()) + \
                    list(options['file_formats']["Video"].items()) + [("other", "other audiovisual file format")]]
            )
            if mode == "val":
            ...
            entry_dict["media"]["audiovisual_format"] = make_multiselect(
                ...
                default=entry_dict["media"]["audiovisual_format"] if mode == "val" else None,
            )
        ...
        if "image" in entry_dict["media"]["category"] or "image" in entry_dict["media"]["text_is_transcribed"]:
            image_format_dict = dict(
                [(fmt, desc) for fmt, desc in list(options['file_formats']["Image"].items()) + [("other", "other image file format")]]
            )
            if mode == "val":
    ...
    with st.expander("Media amounts", expanded=False):
        ...
        instance_type_options = ["", "article", "post", "dialogue", "episode", "book", "webpage", "other"]
        if mode == "val" and entry_dict["media"]["instance_type"] not in instance_type_options:
        ...
        entry_dict["media"]["instance_type"] = make_selectbox(
            ...
            index=instance_type_options.index(entry_dict["media"]["instance_type"]) if mode == "val" else None,
        )
        ...
        instance_count_options = ["", "n<100", "100<n<1K", "1K<n<10K", "10K<n<100K", "100K<n<1M", "1M<n<1B", "n>1B"]
        entry_dict["media"]["instance_count"] = make_selectbox(
            ...
            index=instance_count_options.index(entry_dict["media"]["instance_count"]) if mode == "val" else None,
        )
        ...
        entry_dict["media"]["instance_size"] = make_selectbox(
            ...
            index=instance_size_options.index(entry_dict["media"]["instance_size"]) if mode == "val" else None,
        )
    if mode == "val":
        st.markdown("If you are satisfied with the values for the fields above, press the button below to update and validate the **media type and quantity** section of the entry")
        if make_checkbox(key="validated_media", label="Validate: media type and quantity"):
